...
export class CalendarDay extends Implementation {
  constructor(path, { format = "yyyy-MM-dd", dateFrom, dateTo }) {
    ...
    if (
      this._dateTo &&
      (this._dateTo.length !== 10 || !isValid(parseISO(this._dateTo)))
    ) {
      throw new Error(
  ...
  async validateInput({ resolvedData, addFieldValidationError }) {
    ...
    if (parsedValue) {
      if (
        parseISO(this._dateFrom) &&
        compareAsc(parseISO(this._dateFrom), parsedValue) === 1
      ) {
        addFieldValidationError(
          `Value is before earliest allowed date: ${this._dateFromString}.`,
          {
            value: resolvedData[this.path],
      ...
      if (
        parseISO(this._dateTo) &&
        compareDesc(parseISO(this._dateTo), parsedValue) === 1
      ) {
        addFieldValidationError(
          `Value is after latest allowed date: ${this._dateToString}.`,
          {
            value: resolvedData[this.path],
...
export class MongoCalendarDayInterface extends CommonCalendarInterface(
  MongooseFieldAdapter,
) {
  addToMongooseSchema(schema) {
    const validator = (a) =>
      typeof a === "string" && a.length === 10 && parseISO(a);
    const schemaOptions = {
      type: String,
      validate: {
