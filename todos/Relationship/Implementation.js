...
export class Relationship extends Implementation {
  ...
  tryResolveRefList() {
    ...
    if (!refList) {
      throw new Error(
        `Unable to resolve related list '${refListKey}' from ${listKey}.${path}`,
      );
    }
  ...
  gqlQueryInputFields({ schemaName }) {
    ...
    if (this.many) {
      ...
      return [
        `${this.path}: ${refList.gqlNames.whereInputName}`,
    ...
    if (this.many) {
      ...
      return {
        [this.path]: (item, _, context, info) => {
          ...
          const id =
            item &&
            (item[this.adapter.idPath] ||
              (item[this.path] && item[this.path].id));
          if (!id) {
            return null;
          }
  ...
  async resolveNestedOperations(
    ...
    if (this.many) {
      ...
      currentValue =
        item &&
        (item[this.adapter.idPath] || (item[this.path] && item[this.path].id));
...
export class MongoRelationshipInterface extends MongooseFieldAdapter {
  ...
  addToMongooseSchema(schema, _mongoose, rels) {
    ...
    if (!this.field.many) {
      ...
      schema.add({
        [this.path]: this.mergeSchemaOptions({ type, ref: refListKey }, config),
      });
    }
  }
