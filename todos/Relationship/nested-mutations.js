...
import { intersection, pick } from "@ocopjs/utils";
...
function validateInput({ input, target, many }) {
  ...
  if (many) {
    ...
    validInputMutations = validInputMutations.filter(
      (mutation) =>
        mutation === "disconnectAll" || Array.isArray(input[mutation]),
    );
  } else {
    validInputMutations = validInputMutations.filter(
      (mutation) =>
        mutation === "disconnectAll" || Object.keys(input[mutation]).length,
    );
  }
  ...
  if (
    !many &&
    validInputMutations.includes("create") &&
    validInputMutations.includes("connect")
  ) {
    throw new ParameterError({
