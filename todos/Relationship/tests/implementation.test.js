...
class MockList {
  ...
  // The actual implementation in `@ocop/ocop/List/index.js` returns
...
describe("Type Generation", () => {
  ...
  test("to-single relationship nested mutation input", () => {
    ...
    expect(
      gql(relationship.getGqlAuxTypes({ schemaName }).join("\n")),
    ).toMatchObject({
      definitions: [
        {
...
describe("Referenced list errors", () => {
  ...
  ["gqlOutputFields", "gqlQueryInputFields", "gqlOutputFieldResolvers"].forEach(
    (method) => {
      describe(`${method}()`, () => {
        const schemaName = "public";
        ...
        test("throws when list not found", async () => {
          ...
          expect(
            asyncify(() => relMany[method]({ schemaName })),
          ).rejects.toThrow(/Unable to resolve related list 'Zip'/);
        });
