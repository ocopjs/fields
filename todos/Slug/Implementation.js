...
const generateSlug = (valueToSlugify) =>
...
export class SlugImplementation extends Text {
  constructor(
    ...
    if (typeof regenerateOnUpdate !== "boolean") {
      throw new Error(
        `The 'regenerateOnUpdate' option on ${listAndFieldPath} must be true/false`,
      );
    }

    if (typeof alwaysMakeUnique !== "boolean") {
      throw new Error(
        `The 'alwaysMakeUnique' option on ${listAndFieldPath} must be true/false`,
      );
    }
    ...
    if (from) {
      if (typeof from !== "string") {
        ...
        throw new Error(
          `The 'from' option on ${listAndFieldPath} must be a string`,
        );
      }
      ...
      generateFn = ({ resolvedData, existingItem }) => {
        // Ensure we generate on a complete object (because `resolvedData` may
        // only be partial)
        const { id, name, title, ...fields } = {
          ...existingItem,
          ...resolvedData,
        };
        const valueToSlugify =
          name || title || findFirstNonEmptyStringValue(fields);
        if (!valueToSlugify) {
          throw new Error(
    ...
    const isUniqueCalculated =
      typeof isUnique === "undefined" ? true : isUnique;
